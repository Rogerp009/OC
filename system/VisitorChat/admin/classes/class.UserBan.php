...
class UserBan {
	...
	*/
	public static function ListBanned() {
		...
		try {
			...
			$sth = DB::prep("SELECT id, INET_NTOA(ip) as ip, host FROM messaging_ban");
	...
	*/
	public static function BanDelete($id) {
		...
		try {
			...
			$sth = DB::prep("DELETE FROM messaging_ban WHERE id = :id");
	...
	*/
	private static function GetUser($id) {
			...
			$sth = DB::prep("SELECT INET_NTOA(ip) as ip FROM messaging_users WHERE user_id = :id");
			$sth->bindParam(":id", $id, PDO::PARAM_INT);

...
class UsersAndGroups extends Errors {
	...
	*/
	public static function ListUsers() {
		...
		try {
			...
			$sth = DB::prep("SELECT m1.id,m1.username,m2.title FROM messaging_admin m1 LEFT JOIN messaging_groups m2 ON m1.group = m2.id");
	...
	*/
	public static function GroupDelete($group_id) {
		...
		try {
			...
			$sth = DB::prep("DELETE FROM messaging_groups WHERE id = :id");
	...
	*/
	public static function UserDelete($user_id) {
		...
		try {
			...
			$sth = DB::prep("DELETE FROM messaging_admin WHERE id = :id");
	...
	*/
	private function CheckGroup() {
		...
		try {
			...
			$sth = DB::prep("SELECT COUNT(*) as c FROM messaging_groups WHERE id = :id");
	...
	*/
	function UserEdit() {
		...
		try {
			$sth = DB::prep("UPDATE messaging_admin SET pass = sha1(:pass), `group` = :group WHERE id = :id");
	...
	*/
	function UserInsert() {
		...
		try {
			$sth = DB::prep("INSERT INTO messaging_admin (username,pass,`group`) VALUES (:username,sha1(:pass),:group)");
	...
	*/
	function GroupEdit() {
		...
		try {
			$sth = DB::prep("UPDATE messaging_groups SET title = :title, groups = :groups, banned = :banned, history = :history WHERE id = :id");
	...
	*/
	function GroupInsert() {
		...
		try {
			$sth = DB::prep("INSERT INTO messaging_groups (title,groups,banned,history)
	...
	*/
	public static function GetUser($id) {
		...
		try {
			...
			$sth = DB::prep("SELECT * FROM messaging_admin WHERE id = :id");

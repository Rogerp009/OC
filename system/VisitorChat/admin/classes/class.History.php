...
class History {
	...
	*/
	public static function ListHistory() {
		...
		try {
			...
			$sth = DB::prep("SELECT *, INET_NTOA(from_ip) as from_ip, SEC_TO_TIME(TIMESTAMPDIFF(SECOND, MIN(time), MAX(time))) as time2, DATE_FORMAT(time,'%Y-%m-%d') as time FROM messaging_history GROUP BY email,sess ORDER BY id DESC");
	...
	*/
	public static function SearchHistory($search) {
		...
		try {
			...
			$sth = DB::prep("SELECT *, INET_NTOA(from_ip) as from_ip, SEC_TO_TIME(TIMESTAMPDIFF(SECOND, MIN(time), MAX(time))) as time2, DATE_FORMAT(time,'%Y-%m-%d') as time FROM messaging_history WHERE sess IN (SELECT sess FROM messaging_history WHERE MATCH (msg) AGAINST (:search)) GROUP BY sess");
	...
	*/
	public static function GetConversation($session_id) {
		...
		try {
			...
			$sth = DB::prep("SELECT * FROM messaging_history WHERE session = :sess");
	...
	*/	
    public static function GetHistorySess($session, $email)
    {
        ...
        try {
            $sth = DB::prep("SELECT TIME_FORMAT(time, '%k:%i') as time,user as user_nick, admin, type, msg FROM messaging_history WHERE sess = :sess AND email = :email ORDER BY time");

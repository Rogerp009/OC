...
class RegisterUser
{
	...
	*/
	function Register($name,$email) {
		...
		$sth = DB::prep("INSERT INTO messaging_users (nick,email,ip,sess) VALUES ( :guest, :email, INET_ATON(:ip), :sess )");
	...
	*/
	private function AvailableAdmin() {
		...
		$sth = DB::prep("SELECT id,username FROM messaging_admin WHERE time > (NOW() - INTERVAL 30 SECOND) ORDER BY RAND()");
	...
	*/
	function WelcomeUser($user, $admin) {
		...
		$sth = DB::prep("INSERT INTO messaging (msg,user,to_user,type) VALUES (:msg, :user_id, :this_admin,'admin') ");
	...
	private function UpdateUserOnline() {
		...
		$sth = DB::prep("INSERT INTO messaging_users (user_id,nick,email,ip,sess) VALUES ( :new_id, :guest, :email, INET_ATON(:ip), :sess ) ON DUPLICATE KEY UPDATE time = NOW() ");

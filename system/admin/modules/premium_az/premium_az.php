...
if(isset($_GET['display']) && !empty($_GET['display']) && is_numeric($_GET['display']) && $_GET['display'] >= 0 && $_GET['display'] <= 1000)
{
	...
	$_SESSION['display13'] = $display;
	$limit = $_SESSION['display13'];
...
// ------------------------------------------------------------
$query = "SELECT COUNT(*) AS TotalCount FROM $tableName WHERE UserName LIKE '$az_letter%' AND IsPremium =1";
...
// ------------------------------------------------------------
$query1 = "SELECT UserId, UserName, Email, IsApproved, IsLockedOut, IsLoggedIn, CreateDate FROM $tableName WHERE UserName LIKE '$az_letter%' AND IsPremium = 1 ORDER BY $default_sorting LIMIT $start, $limit";
